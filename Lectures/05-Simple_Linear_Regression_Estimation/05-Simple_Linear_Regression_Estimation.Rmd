...
- Midterm 1 is on Wednesday January 29th
- Will have extra office hours, announced through email
...
Instead, we must estimate the population parameters using our $\textit{data sample}$.


$$
 \text{Crime}_i = \text{b}_1 + \text{b}_2\text{Police}_i + e_i
$$


But, is $b_1 = \beta_1$ and $b_2 = \beta_2$?

---

Using our estimates, we can make predictions:
...
Step 1. Replace $\sum_{i=1}^n \hat{u}_i^2$ with an equivalent expression involving $\hat{\beta_1}$ and $\hat{\beta_2}$.

--

Step 2. Take partial derivatives of our RSS expression with respect to $\hat{\beta_1}$ and $\hat{\beta_2}$ and set each one equal to zero (first-order conditions).

--

Step 3. Use the first-order conditions to solve for $\hat{\beta_1}$ and $\hat{\beta_2}$ in terms of data on $Y_i$ and $X_i$.

--

Step 4. Check second-order conditions to make sure we found the $\hat{\beta_1}$ and $\hat{\beta_2}$ that minimize RSS.
