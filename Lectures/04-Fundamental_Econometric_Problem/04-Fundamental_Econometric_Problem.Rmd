...
# Notes directory
dir_slides <- "~/GitHub/Intro_Econometrics_F19/Lectures/04-Fundamental_Econometric_Problem/"
# Knitr options
opts_chunk$set(
  comment = "#>",
  fig.align = "center",
...
geom_text(aes(label = id), color = "grey45", size = 5, family = "sans") +
theme_void() +
coord_equal()
```
---
...
g <- ggplot(data = med_df,
  aes(x, y)
) +
geom_tile(aes(fill = q), color = "white", size = 0.2) +
# geom_text(aes(label = trt)) +
#geom_emoji(sample_n(med_df, 1), emoji = "1f48a") +
  geom_emoji('pill', x = 2, y = 5, size = 5) +
scale_fill_viridis(option = "magma", direction = -1) +
theme_void() +
theme(legend.position = "none") +
coord_equal()
...
geom_tile(aes(fill = q), color = "white", size = 0.2) +
# geom_text(aes(label = trt)) +
geom_emoji(aes(sample_n(med_df, 2)), emoji = 'pill') +
scale_fill_viridis(option = "magma", direction = -1) +
theme_void() +
theme(legend.position = "none") +
coord_equal()
